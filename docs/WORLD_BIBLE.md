...
**Mobs** (`js/entities/enemy.js`): **zombie** (hp 6, dmg 1), **skeleton** — including `dungeon_skeleton`, a bow archer (hp 5, dmg 1) that fires arrow projectiles — and **`cave_spider`** (hp 2, dmg 1, fast — the L2 swarm member). The cave spider also has a **ranged web attack**: it periodically spits a web projectile (modeled on the skeleton's bow signal, staggered so the swarm doesn't fire in unison) that, on contact, **freezes the player for 2 seconds** — fully immobilized but still vulnerable to melee, with a re-stick guard so the swarm can't chain-freeze. Missed webs splat on the floor/wall and fade out. "Boss" is expressed through the registry's `boss: { type, spawn, clearedFlag }` entry, not a separate framework (see §7.4).

**NPCs** (`js/data/npcs.js`, `characters.js`): villagers — farmer, librarian, shopkeeper, alchemist, a home NPC, and the **Well Keeper** (appears by the well once the Ender Pearl is found). Drawn via `drawCharacter`. L2 also hides a non-villager follower — **Spud, the secret potato pet** (see §6) — drawn by `drawPotatoPet`.
...
|---|---|---|---|
| **L2 Lush Caverns** ✅ *shipped* | glow berries, moss, big dripleaf (traversal platform), clay, cave water, vines, the Great Chasm, hook anchors | cave-spider swarm (fast, low-HP, many; ranged web-freeze attack) → swarm mini-boss | water gaps, the chasm (grapple-only crossing), web-freeze (2s immobilize) |
...
- **Every level hides at least one secret.** A reward for exploring off the critical path — an optional-path stash of **emeralds or an item** (never a heart container). L1 set the precedent (the forest secret bush → 20 emeralds; L2's hidden glow-berry stash → 25); every new level carries the tradition. This is a hard requirement, not flavor (see LEVEL_SPEC §2 and criterion G10). *Bonus secrets beyond the required one are encouraged* — L2 also tucks **Spud, the secret potato pet**, in the lower entry grotto: walk onto it to trigger a discovery dialogue, after which it trails the player around the caverns via a breadcrumb buffer. (A pet is a flavor easter-egg, not the G10-qualifying reward — the glow-berry emeralds are.)
