...
2. **New tiles**: add IDs in `js/data/tileTypes.js` starting at the next free integer (currently **48**; use your reserved block in LEVEL_SPEC §6). **Every new ID MUST get a `tileProps` entry** — a missing entry renders nothing and is treated as solid (an invisible wall). Draw it in `js/world/tilemap.js` `drawTile` using the existing fill-rect + speckle idiom (no images).
